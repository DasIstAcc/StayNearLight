...
public class ThirdPersonMotor : MonoBehaviour
{
    ...
    [Tooltip("How much time does dash take")]
    ...
    internal float dashCounter;                         // used to count time dash is performed
    internal float currDashCooldown = 0f;               // current cooldown of dash
    ...
    internal Transform camera_look;                     // used to know the direction of the camera
    ...
    private void ControlDashBehavior()
    {
        if (currDashCooldown > 0) currDashCooldown -= Time.deltaTime;
        ...
        Vector3 vec = input.x * camera_look.right + input.z * camera_look.forward;
        vec.y = 0f;
        _rigidbody.AddForce(vec.normalized * dashSpeed * 10000);
    ...
    protected virtual void CheckGround()
    {
        ...
        {
            isGrounded = true;
            if (!isJumping && !isDashing && groundDistance > 0.05f)
